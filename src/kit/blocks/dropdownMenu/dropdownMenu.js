class DropdownMenu {
  ...
  init(
    ...
    menuHeader.addEventListener('click', () => {
      ...
      menuHeader.classList.toggle('dropdown-menu__header_opened');
    });
    if (!isClosed) {
      menuHeader.classList.add('dropdown-menu__header_opened')
    }
  ...
  _buildItem(el, i) {
    ...
    const listItem = `
          <h3>${el.name}</h3>
          <span class="dropdown-menu__counter">
            <button ${el.count === 0 ? 'disabled' : ''}>-</button>
            <h3 class="dropdown-menu__count">${el.count}</h3>
            <button>+</button>
          </span>
    `.replace(/>\s+</g, '><');
    // </div>
    ...
    let counter = item.querySelector('.dropdown-menu__counter');
  ...
  _initMenuButtons(withCancel, withConfirm) {
    if (withCancel) {
      let cancelButton = this._menu.querySelector('.dropdown-menu__cancel');
    ...
    if (withConfirm) {
      let confirmButton = this._menu.querySelector('.dropdown-menu__confirm');
  ...
  _clear() {
    ...
    this._menu.querySelectorAll('.dropdown-menu__counter').forEach(el => {
